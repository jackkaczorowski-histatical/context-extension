(() => {
  ...
  async function startRecording(streamId) {
    try {
      console.log('[CONTENT] Starting getUserMedia with streamId:', streamId);
      ...
      mediaRecorder.ondataavailable = async (event) => {
        if (event.data.size > 0) {
          console.log('[CONTENT] Audio chunk captured, blob size:', event.data.size, 'bytes');
          try {
            const base64 = await blobToBase64(event.data);
            chrome.runtime.sendMessage({ type: 'AUDIO_CHUNK', audio: base64 });
          } catch (e) {
            console.error('[CONTENT] Failed to send audio chunk:', e.name, e.message);
          }
        }
      };

      mediaRecorder.onerror = (event) => {
        console.error('[CONTENT] MediaRecorder error:', event.error.name, event.error.message);
      };

      mediaRecorder.start(4000); // 4-second chunks
      console.log('[CONTENT] MediaRecorder started, mimeType:', mediaRecorder.mimeType);
    } catch (err) {
      console.error('[CONTENT] getUserMedia error:', err.name, err.message);
    }
  }
